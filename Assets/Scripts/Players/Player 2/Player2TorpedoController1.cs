...
public class Player2TorpedoController1 : MonoBehaviour
{
    ...
    {
        Vector3 direction = (Player2Controller.player2.torpedoTargetPosition[TORPEDO_1] - transform.position).normalized;

        transform.position += direction * torpedoSpeed * Time.deltaTime;

        if (Vector3.Distance(Player2Controller.player2.torpedoTargetPosition[TORPEDO_1], transform.position) <= 0.1f)
        {
            gameObject.SetActive(false);

            Player2Controller.player2.torpedoTarget[TORPEDO_1].gameObject.SetActive(false);
        }
    }
