...
public class Player2TorpedoController2 : MonoBehaviour
{
    public float torpedoSpeed;

    private const int TORPEDO_2 = 1;
    ...
    {
        Vector3 direction = (Player2Controller.player2.torpedoTargetPosition[TORPEDO_2] - transform.position).normalized;

        transform.position += direction * torpedoSpeed * Time.deltaTime;

        if (Vector3.Distance(Player2Controller.player2.torpedoTargetPosition[TORPEDO_2], transform.position) <= 0.1f)
        {
            gameObject.SetActive(false);

            Player2Controller.player2.torpedoTarget[TORPEDO_2].gameObject.SetActive(false);
        }
    }
