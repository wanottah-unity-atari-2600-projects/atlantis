...
public class Player2TorpedoController4 : MonoBehaviour
{
    public float torpedoSpeed;

    private const int TORPEDO_4 = 3;
    ...
    {
        Vector3 direction = (Player2Controller.player2.torpedoTargetPosition[TORPEDO_4] - transform.position).normalized;

        transform.position += direction * torpedoSpeed * Time.deltaTime;

        if (Vector3.Distance(Player2Controller.player2.torpedoTargetPosition[TORPEDO_4], transform.position) <= 0.1f)
        {
            gameObject.SetActive(false);

            Player2Controller.player2.torpedoTarget[TORPEDO_4].gameObject.SetActive(false);
        }
    }
